...
| I51 | `@carbon-labs/react-plane-stack-3d` 0.10.0 | 3D plane stack visualization (three.js); React, live (2026-09-03); peers `react ^18.0.0` (`sources/round3/labs-tech.json`; L row) · **pending S20: peers React 18 only, conflicts with React 19; not counted as confirmed** |
...
| S20 | `@carbon-labs/react-plane-stack-3d` (I51) peers `react ^18.0.0` / `react-dom ^18.0.0` only, while the rest of the stack targets React 19 (`sources/round3/labs-tech.json`) | Pending Hleb. (a) Include and override the peer range: consequence, runs on an untested React major, may break. (b) Keep it out until Labs widens the peer to React 19: consequence, one Labs package missing despite the Labs rule. (c) Contribute or wait for an upstream React 19 peer update, then include: consequence, depends on Labs timing. | new |
